...
namespace FSO.IDE
{
    public partial class ObjectWindow : Form, IffResWindow
    {
        ...
        {
            var sg = GameContent.Get.WorldObjectGlobals.Get(SemiglobalName);
            if (sg == null)
            {
        ...
        private void DeleteOBJD_Click(object sender, EventArgs e)
        {
            ...
            if (confirm == DialogResult.Yes)
            {
                GameContent.Get.Changes.BlockingResMod(new ResAction(() =>
                {
                    ...
                    GameContent.Get.Changes.ChunkChanged(ActiveObj.OBJ);
                    GameContent.Get.WorldObjects.RemoveObject(ActiveObj.OBJ.GUID);
                }));
