...
using FSO.LotView.Diagnostics;
using FSO.LotView.Utils;
using FSO.Content;
using FSO.Files.RC;

namespace FSO.IDE.Common.Diagnostics
{
    public class UI3DDGRP : UIInteractiveDGRP
    {
        ...
        public UI3DDGRP(uint guid) : base(guid)
        {
            ...
            {
                Position = new Vector3(-0.5f, -0.1f, -0.5f)
            };
            Scene.Add(cube);
        ...
        public override void Update(UpdateState state)
        {
            ...
            if ((bool)state.SharedData["ExternalDraw"] && TargetTile?.WorldUI is ObjectComponent)
            {
                ...
                foreach (var obj in TargetTile.MultitileGroup.Objects)
                {
                    ...
                    c.Mesh = (dgrp == null) ? null : GameContent.Get.RCMeshes.Get(dgrp, obj.Object.OBJ); //new DGRP3DMesh(((ObjectComponent)obj.WorldUI).DGRP, obj.Object.OBJ, GameFacade.GraphicsDevice, null);
                    Scene.Add(c);
